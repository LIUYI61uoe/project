...
class RNN(object):
	...
	def predict(self, x):
		...
		#s[-1]=np.zeros(self.hidden_dims)
		y = np.zeros((len(x), self.out_vocab_size))
		
		for t in range(len(x)):
			x_one = make_onehot(x[t],self.vocab_size)
			#s[t] = sigmoid(self.V[:,x_one]+self.U.dot(s[t-1]))#np.dot
			s[t]= sigmoid(np.dot(self.V,x_one)+np.dot(self.U,s[t-1]))
	...
	def acc_deltas(self, x, d, y, s):
		...
		g_polyder = np.ones(len(x))
		f_polyder = np.array([p*(np.ones(len(p))-p) for p in s])
		for t in reversed(range(len(x))):
			
			
			d_one = make_onehot(d[t],self.out_vocab_size)
			p_out =(d_one- y[t])*g_polyder[t]
			self.deltaW += np.outer(p_out,s[t])
			
			p_in = (np.dot(self.W.T,p_out))*f_polyder[t]
			x_one = make_onehot(x[t],self.vocab_size)
			self.deltaV += np.outer(p_in,x_one)
			self.deltaU += np.outer(p_in,s[t-1])
	...
	def acc_deltas_np(self, x, d, y, s):
		...
		d_one = make_onehot(d[t],self.out_vocab_size)
		p_out =(d_one- y[t])*g_polyder[t]
		self.deltaW += np.outer(p_out,s[t-1])
		p_in = (np.dot(self.W.T,p_out))*f_polyder[t-1]
		x_one = make_onehot(x[t],self.vocab_size)
		self.deltaV += np.outer(p_in,x_one)
		if len(s)>2:
			self.deltaU += np.outer(p_in,s[t-2])
		else:
			self.deltaU += np.outer(p_in,s[t-1])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		g_polyder = np.ones(len(x))
		f_polyder = np.array([p*(np.ones(len(p))-p) for p in s])  # no loop t = len(x)-1
		for t in reversed(range(len(x))):
			p_in=np.zeros((len(x),self.hidden_dims))
			d_one = make_onehot(d[t],self.out_vocab_size)
			p_out =(d_one- y[t])*g_polyder[t]
			self.deltaW += np.outer(p_out,s[t])
			for step in range(0,steps+1):
				if step==0:
					p_in[t-step] = np.dot(self.W.T,p_out)*f_polyder[t]
				elif t-step<0:
					break
				else:
					p_in[t-step] = np.dot(self.U.T,p_in[t-step+1])*f_polyder[t-step]
				x_one = make_onehot(x[t-step],self.vocab_size)
				self.deltaV += np.outer(p_in[t-step],x_one)
				self.deltaU += np.outer(p_in[t-step],s[t-step-1])
		'''
		accumulate updates for V, W, U
		back propagation through time (BPTT)compute mean loss. Giv
		
		this should not update V, W, U directly. instead, use deltaV, deltaW, deltaU to accumulate updates over time
		
		x		list of words, as indices, e.g.: [0, 4, 2]
		d		list of words, as indices, e.g.: [4, 2, 3]
		y		predicted output layer for x; list of prob7ability vectors, e.g., [[0.3, 0.1, 0.1, 0.5], [0.2, 0.7, 0.05, 0.05] [...]]
				should be part of the return value of predict(x)
		s		predicted hidden layer for x; list of vectors, e.g., [[1.2, -2.3, 5.3, 1.0], [-2.1, -1.1, 0.2, 4.2], [...]]
				should be part of the return value of predict(x)
...
##########################
	...
	def acc_deltas_bptt_np(self, x, d, y, s, steps):
		...
		t= len(x)-1
		p_in=np.zeros((len(x),self.hidden_dims))
		d_one = make_onehot(d[-1],self.out_vocab_size)
		p_out =(d_one- y[t])*g_polyder[t]
		self.deltaW += np.outer(p_out,s[t])
		for step in range(0,steps+1):
			if step==0:
	...
	def compute_loss(self, x, d):
		loss = 0
		y,s =self.predict(x)
		for l in np.arange(len(d)):
			
			d_j =make_onehot(d[l],self.vocab_size)

			
			loss += -1*np.sum(np.dot(d_j,np.log(y[l])))
		...
		#y,s = self.predict(x)
		y=softmax(x)
		loh_likelihod = -np.log(y[range(m),y])
		loss = np.sum(loh_likelihod)
		return loss
		'''
	...
	def compute_loss_np(self, x, d):
		...
		y,s =self.predict(x)
		#l=len(d)-1
		d_j =make_onehot(d[0],self.vocab_size)
		loss = -1*np.dot(d_j,np.log(y[-1]))
	...
	def train(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
		...
		for epoch in range(epochs):
			...
			# use random sequence of instances in the training \n### computing binary prediction loss"set (tries to avoid local maxima when training on batches)
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		run_loss = model.train( X_train, D_train, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True)
		mean_loss= model.compute_mean_loss(X_dev,D_dev)
		test_loss = model.compute_mean_loss(X_test,D_test)

		adjusted_loss = adjust_loss(test_loss, fraction_lost, q, mode='basic')
...
#		run_loss = -1
		...
		print("mean loss: %.03f" % np.exp(mean_loss))
		print("mean loss: %.03f" % mean_loss)
	...
	if mode == "train-np":
		...
		model = RNN(vocab_size,hdim,vocab_size)
		#mean_loss= model.compute_mean_loss(X_dev,D_dev)
		run_loss = model.train_np( X_train, Y_train, X_dev, D_dev, epochs=10, learning_rate=0.1, anneal=5, back_steps=5, batch_size=100, min_change=0.0001, log=True)
		#adjusted_loss = adjust_loss(mean_loss, fraction_lost, q, mode='basic')
		#test_loss = sum([RNN.compute_loss(X_test[i], D_test[i]) for i in range(len(X_test))]) / sum([len(d) for d in D_test])
#    adjusted_loss = adjust_loss(mean_loss, fraction_lost, q, mode='basic')
		loss = sum([RNN.compute_loss_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / len(D_test)
		acc = sum([RNN.compute_acc_np(X_test[i], D_test[i]) for i in range(len(X_test))])/ len(D_test)
		print("Loss: %.03f" % loss)
